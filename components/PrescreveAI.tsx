...
7. "guiaPassos": ARRAY de strings numeradas com os passos de execução precisos.`;
      ...
      console.error(err);
      setError(`Falha técnica na geração. Verifique a ligação e tente novamente. Detalhes: ${err.message || String(err)}`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 font-sans antialiased p-4 sm:p-8 flex flex-col items-center overflow-y-auto relative">
      ...
      {/* CONTROLES */}
      <div className="max-w-4xl w-full grid grid-cols-1 gap-4 mb-4">
        {/* Environment selected by default, only showing gender choice */}
        <div className="flex bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm max-w-md mx-auto w-full">
      ...
      {/* Jersey selection hidden (South Africa 26 is default) */}
          ...
          <div className="relative bg-white border border-slate-100 rounded-[3rem] p-2 flex items-center shadow-lg transition-all">
            ...
            {searchTerm.trim().length > 0 && (
              <button 
                ...
                className="flex ml-2 mr-2 bg-gradient-to-r from-red-600 to-orange-500 text-white px-6 md:px-8 py-3 md:py-4 rounded-full font-bold hover:scale-[1.02] active:scale-95 transition-all items-center gap-2 disabled:opacity-50 shadow-lg shrink-0"
              >
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {!isGenerating && (
              <div className="px-4 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { const link = document.createElement('a'); link.href = generatedData.image; link.download = 'PrescreveAI.png'; link.click(); }} className="bg-white text-slate-700 px-6 py-4 rounded-2xl font-bold flex items-center gap-2 hover:bg-slate-50 transition-all border border-slate-200 shadow-sm"><Download size={20} className="text-red-500" /> Salvar</button>
                  <button onClick={handleGenerateInsight} disabled={isGeneratingInsight} className="bg-gradient-to-r from-red-600 to-orange-500 text-white px-6 py-4 rounded-2xl font-bold flex items-center gap-2 hover:scale-[1.02] active:scale-95 transition-all shadow-lg">
            ...
            {performanceInsight && !isGenerating && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-in">
                <div className="bg-red-600 text-white p-6 rounded-[2rem] shadow-xl relative overflow-hidden group">
            ...
            <div className="bg-white border border-slate-100 rounded-[2.5rem] shadow-xl p-8 h-fit relative overflow-hidden">
               ...
               {isGenerating ? (
                 ...
                 <div className="space-y-6 text-sm font-medium text-slate-700 leading-relaxed relative z-10">
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 shadow-sm"><p className="font-black text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-500 uppercase tracking-widest text-xs mb-2">AGONISTAS</p><p>{generatedData.agonistas}</p></div>
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 shadow-sm"><p className="font-black text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-500 uppercase tracking-widest text-xs mb-2">SINERGISTAS</p><p>{generatedData.sinergistas}</p></div>
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 shadow-sm"><p className="font-black text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-500 uppercase tracking-widest text-xs mb-2">CINESIOLOGIA</p><p>{generatedData.cinesiologia}</p></div>
                    <div className="p-5 border border-red-100 bg-red-50/30 rounded-2xl italic font-bold text-slate-600">"{generatedData.citacao}"</div>
                 </div>
               )}
            </div>
