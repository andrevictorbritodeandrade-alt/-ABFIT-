    ...
    <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-2xl flex flex-col p-6 animate-in fade-in zoom-in-95 duration-500 overflow-y-auto custom-scrollbar text-left">
      <header className="flex justify-between items-center mb-8 sticky top-0 z-50 py-2">
        <div className="flex flex-col">
          ...
          <h2 className="text-3xl font-black italic uppercase text-white tracking-tighter leading-none">{ex.name}</h2>
        </div>
        ...
        <div className="relative aspect-video w-full bg-zinc-900 rounded-[2.5rem] overflow-hidden border border-white/10 shadow-3xl group">
          {ex.thumb ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-4">
               ...
               <p className="text-[10px] font-black uppercase text-zinc-700 tracking-widest">IA Processando Biomecânica...</p>
            </div>
          )}
...
// Subcomponente para o Card de Exercício - Reestilizado para ser menos "embolado"
// Fix: Added optional key to props interface to satisfy TypeScript JSX validation
    ...
    return (
      <div className="p-6 pb-48 text-white overflow-y-auto h-screen text-left custom-scrollbar bg-black animate-in fade-in">
        <header className="flex items-center gap-4 mb-10">
          ...
          <h2 className="text-xl font-black italic uppercase tracking-tighter">Planilhas de Treino</h2>
        </header>
        <div className="space-y-4">
          {user.workouts?.map(w => (
            <Card key={w.id} className="p-8 bg-zinc-900/50 border-white/5 flex justify-between items-center group cursor-pointer hover:border-red-600/20 shadow-2xl rounded-[3rem]" onClick={() => startSession(w)}>
              <div>
                <h4 className="text-2xl font-black italic uppercase text-white tracking-tighter group-hover:text-red-600 transition-colors">{w.title}</h4>
    ...
    <div className="p-6 pb-48 text-white overflow-y-auto h-screen text-left custom-scrollbar bg-black animate-in fade-in duration-500">
      ...
      <header className="flex items-center justify-between mb-8 sticky top-0 bg-black/90 backdrop-blur-md z-40 py-6 -mx-6 px-6 border-b border-white/5">
        <div className="flex items-center gap-4">
           <button onClick={() => { if(confirm("Sair da sessão?")) { localStorage.clear(); location.reload(); } }} className="p-3 bg-zinc-800 rounded-2xl text-zinc-500">
              <ArrowLeft size={20}/>
           </button>
           <div className="flex flex-col">
      ...
      {/* DASHBOARD DE PROTOCOLO INTEGRADO */}
      {workoutStats && (
        <div className="mb-8 animate-in slide-in-from-top-4 duration-700">
           <Card className="bg-zinc-900/40 border-white/5 p-6 flex items-center justify-between backdrop-blur-xl rounded-[2.5rem] shadow-3xl">
              <div className="flex items-center gap-4">
                 ...
                 <div className="flex flex-col">
                    <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest italic mb-1">Início</span>
                    <span className="text-xs font-black italic text-white">{workoutStats.startDate}</span>
                 </div>
              </div>
              <div className="flex gap-10">
                 <div className="text-center">
                    <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest italic mb-1 block">Sessões</span>
                    <div className="flex items-baseline gap-1">
                       <span className="text-3xl font-black text-white italic tracking-tighter leading-none">{workoutStats.completed}</span>
                 ...
                 <div className="text-right">
                    ...
                    <div className="flex items-center gap-2 justify-end">
                       ...
                       <span className={`text-xs font-black italic uppercase ${workoutStats.completed >= workoutStats.total - 2 ? "text-amber-500" : "text-zinc-400"}`}>
      ...
      {/* LISTA DE EXERCÍCIOS - Menos embolada */}
      <div className="space-y-4">
  ...
  return (
    <div className="p-6 pb-48 text-white overflow-y-auto h-screen text-left custom-scrollbar bg-black animate-in fade-in">
      <header className="flex items-center gap-4 mb-10 sticky top-0 bg-black/80 backdrop-blur-md z-40 py-4 -mx-6 px-6 border-b border-white/5">
        ...
        <h2 className="text-xl font-black italic uppercase tracking-tighter text-white">Avaliação PhD</h2>
      </header>
        ...
        {student.physicalAssessments && student.physicalAssessments.length > 0 ? (
          student.physicalAssessments.map(pa => (
            <Card key={pa.id} className="p-8 bg-zinc-900 border-zinc-800 rounded-[2.5rem] shadow-3xl">
               <div className="flex justify-between items-start mb-6">
                  <h4 className="text-2xl font-black italic uppercase text-white tracking-tighter leading-none">{new Date(pa.data).toLocaleDateString('pt-BR')}</h4>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="p-5 bg-black/60 rounded-3xl border border-white/5">
                    ...
                    <p className="text-3xl font-black text-red-600 italic tracking-tighter">{pa.peso}KG</p>
                  </div>
                  <div className="p-5 bg-black/60 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-zinc-600 uppercase tracking-widest italic mb-2">Gordura Bio</p>
                    <p className="text-3xl font-black text-red-600 italic tracking-tighter">{pa.bio_percentual_gordura}%</p>
                  </div>
               </div>
            </Card>
          ))
...
// Fix: Added missing StudentPeriodizationView export
export function StudentPeriodizationView({ student, onBack }: { student: Student, onBack: () => void }) {
  return (
    <div className="p-6 pb-48 text-white overflow-y-auto h-screen text-left custom-scrollbar bg-black animate-in fade-in">
      <header className="flex items-center gap-4 mb-10 sticky top-0 bg-black/80 backdrop-blur-md z-40 py-4 -mx-6 px-6 border-b border-white/5">
        <button onClick={onBack} className="p-2 bg-zinc-900 rounded-full text-white hover:bg-red-600 transition-colors shadow-lg"><ArrowLeft size={20}/></button>
        <h2 className="text-xl font-black italic uppercase tracking-tighter text-white">Periodização PhD</h2>
      </header>
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Card className="mt-10 p-6 bg-zinc-900/50 border-white/5 w-full max-w-sm">
           ...
           <h3 className="text-2xl font-black italic uppercase text-white tracking-tighter">Fase de Acumulação</h3>
...
// Fix: Added missing AboutView export
export function AboutView({ onBack }: { onBack: () => void }) {
  return (
    <div className="p-6 pb-48 text-white overflow-y-auto h-screen text-left custom-scrollbar bg-black animate-in fade-in">
      <header className="flex items-center gap-4 mb-10 sticky top-0 bg-black/80 backdrop-blur-md z-40 py-4 -mx-6 px-6 border-b border-white/5">
        <button onClick={onBack} className="p-2 bg-zinc-900 rounded-full text-white hover:bg-red-600 transition-colors shadow-lg"><ArrowLeft size={20}/></button>
        <h2 className="text-xl font-black italic uppercase tracking-tighter text-white">Sobre a ABFIT</h2>
      </header>
      <div className="space-y-12">
        <div className="text-center">
          <h3 className="text-4xl font-black italic uppercase text-red-600 tracking-tighter">Elite Performance</h3>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-zinc-900 rounded-3xl border border-white/5 text-center">
               ...
               <p className="text-[8px] font-black uppercase text-white">Certificação PhD</p>
            </div>
            <div className="p-4 bg-zinc-900 rounded-3xl border border-white/5 text-center">
               <Shield className="text-red-600 mx-auto mb-2" size={24} />
               <p className="text-[8px] font-black uppercase text-white">Segurança PBE</p>
            </div>
          </div>
        </div>
      </div>
